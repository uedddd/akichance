...
def get_token_struct() -> bytes:
    ...
    return struct.pack(f" pyodbc.Connection:
...
# ---------------------------------------------------------------------------
# seats テーブルの実際のカラム:
#   seat_id, floor_id, seat_name, status, is_active,
...
# Pydantic モデル定義（予約）
# ---------------------------------------------------------------------------
# reservations テーブルの実際のカラム:
#   reservation_id, seat_id, user_id, outlook_event_id,
#   start_datetime, end_datetime, status, notified_at,
#   created_at, updated_at
# ※ user_name / email は reservations テーブルに存在しない
#    → users テーブルに存在する想定
...
def init_db() -> None:
    """起動時のDB接続確認（テーブル構造は変更しない）"""
    try:
        connection = open_connection()
        cursor = connection.cursor()
        # seats テーブルの件数確認（接続テスト）
        ...
        print(f"[DB] 接続成功 / seats: {seats_count}件 / reservations: {res_count}件")
        connection.close()
...
async def negotiate():
    """SignalR 接続トークンを返す。未設定時はフロントが接続スキップできるよう disabled を返す"""
    ...
    if not conn_str:
        ...
        # 500エラーではなく disabled フラグを返してフロント側でハンドリングさせる
...
def assert_time_range(start: datetime, end: datetime) -> None:
    ...
    if end <= start:
        raise HTTPException(
            status_code=400, detail="end_datetime は start_datetime より後にしてください"
        )
...
def is_overlapping(
    ...
    reservations テーブルのカラム名: start_datetime / end_datetime
    キャンセル済みは除外する。
    """
    query = """
        SELECT COUNT(1)
...
def list_seats(
    ...
    query = "SELECT seat_id, floor_id, seat_name, status, is_active, created_at, updated_at, seat_type, capacity, has_monitor FROM seats"
    if active_only:
        query += " WHERE is_active = 1"
    query += " ORDER BY seat_id"
...
def get_seat(
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT seat_id, floor_id, seat_name, status, is_active, created_at, updated_at, seat_type, capacity, has_monitor FROM seats WHERE seat_id = ?",
        (seat_id,),
    )
    row = cursor.fetchone()
    if row is None:
...
def create_seat(
    ...
    cursor.execute(
        """
        INSERT INTO seats
            (floor_id, seat_name, status, is_active, seat_type, capacity, has_monitor,
             created_at, updated_at)
...
def delete_seat(
    ...
    if cursor.fetchone()[0] > 0:
        raise HTTPException(
            status_code=400, detail="この座席には予約が存在するため削除できません"
        )
...
def cancel_reservation(
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT reservation_id, seat_id, user_id, outlook_event_id FROM reservations WHERE reservation_id = ?",
        (reservation_id,),
    )
    row = cursor.fetchone()
    if row is None:
    ...
    cursor.execute(
        "UPDATE reservations SET status = 'cancelled', updated_at = GETDATE() WHERE reservation_id = ?",
        (reservation_id,),
    )
    conn.commit()
